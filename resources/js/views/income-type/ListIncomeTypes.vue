    ...
    <v-layout row wrap>
      <v-flex xs12 sm12 md12 lg12 xl12>
        <modal-delete :message="message" :loading="loading" :remove="remove" @hide="remove = !remove" @deleted="deleted"></modal-delete>
          ...
          <modal-type :modal="modal" @hide="modal = !modal" :data="income_type"></modal-type>
        <v-card flat>
          <v-card-title primary-title>
            <h3 class="headline mb-0">Lista Tipos de Ingreso</h3>
          </v-card-title>
          <v-container fluid>
            <v-layout>
              <v-flex xs12 sm12 md12 lg12 xl12>
                    ...
                    <template slot="items" slot-scope="props">
                      ...
                      <td>
                        <v-btn
                          ...
                          icon class="mx-0" 
                          color="grey"
                          :to="{ name: 'EditIncomeType', params: { id: props.item.id }}"
                        >
  ...
  export default {
    ...
    data () {
      return {        
        ...
        headers: [
          ...
          { text: 'Ingresos', sortable: false, value: 'ingresos', width: "50" },
    ...
    methods: {
      ...
      deleted:async function() {
        this.loading = true
        const response = await IncomeTypeService.deleteIncomeType(`income-types/${this.id}`)
      ...
      getDataFromApi() {
        ...
        return new Promise((resolve, reject) => {
          ...
          IncomeTypeService.getIncomeTypes(this.buildURL())
          .then((response) => {
      ...
      getDetail: async function(id) {
        this.loader = true
        const response = await IncomeTypeService.getIncomeTypes(`income-types/${id}`)
        if (response.status === 200) {
      ...
      buildURL() {
        ...
        return `income-types${page}${rowsPerPage}${filter}`
      }
    }
  }
</script>
